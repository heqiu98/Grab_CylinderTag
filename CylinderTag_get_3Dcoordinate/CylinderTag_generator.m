...
CylinderTagGenerator(12, 2, 100, 12);
...
function plot_tag(ID, ratio)
    ...
    for i=1:size(ID,1)
        background=ones(tag_length,1.5*tag_length/ratio*size(ID,2));%创建全白的背景，全1的矩阵，行数是tag_length，列数是1.5*tag_length/ratio*size(ID,2)
        for j=1:size(ID,2)%size(ID,2)参数决定了每张图中要画多少条黑色条纹
            background=draw(background,j-1,ID(i,j),ratio, i);
        end
        imshow(background)
...
function background=draw(background, cnt, ID_now, ratio, i)
    global tag_length
    ...
    % 定义第一个多边形的顶点坐标  
    points1 = [tag_length/ratio * 1.5 * cnt, 0; ...  
           tag_length/ratio * 1.5 * cnt + tag_length/ratio, 0; ...  
           tag_length/ratio * 1.5 * cnt + tag_length/ratio, block_pos_right-tag_length*white_ratio/2; ...  
           tag_length/ratio * 1.5 * cnt, block_pos_left-tag_length*white_ratio/2];  
  
    % 定义第二个多边形的顶点坐标  
    points2 = [tag_length/ratio * 1.5 * cnt + tag_length/ratio, tag_length;...
            tag_length/ratio * 1.5 * cnt, tag_length; ...  
           tag_length/ratio * 1.5 * cnt, block_pos_left + tag_length*white_ratio/2; ...  
           tag_length/ratio * 1.5 * cnt + tag_length/ratio, block_pos_right + tag_length*white_ratio/2];  
  
    % 合并两个多边形的顶点坐标，因为它们的起始和结束点是相同的  
    all_points = [points1; points2]; % 排除重复的结束点  
  
    % 要写入的文件名  
    filename = sprintf('Pixel_coordinate%d.csv', i);  
  
    % 检查文件是否存在  
    if ~exist(filename, 'file')  
        % 如果文件不存在，创建一个新数组并写入CSV文件  
        csvwrite(filename, all_points);  
    else  
        % 如果文件存在，读取原始数据，追加新数据，然后写回文件  
        originalData = readmatrix(filename); % 读取原始数据  
        combinedData = [originalData; all_points]; % 合并数据  
        writematrix(combinedData, filename); % 写回文件，覆盖原始内容  
    end
end
